...
class TestFineweb(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_memmap_path = os.path.join(
            self.temp_dir, f"fineweb-sample-10BT-{self.tokenizer.name}.bin"
        )
        mock_data = np.arange(10000, dtype=np.uint16)
        mock_memmap = np.memmap(
            self.mock_memmap_path, dtype=np.uint16, mode="w+", shape=10000
        )
    ...
    def test_init_with_invalid_split(self):
        ...
        with self.assertRaises(ValueError):
            Fineweb(tokenizer=self.tokenizer, root_path=self.temp_dir, split="invalid")

    def test_init_with_invalid_tokens_per_sample(self):
        """Test initialization with invalid tokens_per_sample"""
        with self.assertRaises(ValueError):
            Fineweb(
                tokenizer=self.tokenizer, root_path=self.temp_dir, tokens_per_sample=0
            )
    ...
    def test_init_with_invalid_samples_per_epoch(self):
        ...
        with self.assertRaises(ValueError):
            Fineweb(
                tokenizer=self.tokenizer, root_path=self.temp_dir, samples_per_epoch=0
            )

    def test_tokenize_method(self):
        """Test the tokenize method"""
        dataset = Fineweb(tokenizer=self.tokenizer, root_path=self.temp_dir)
...
class TestChatMLTokenizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.chatml_tokenizer = ChatMLTokenizer(
            tokenizer=self.tokenizer, max_tokens_per_sample=100
        )
    ...
    def test_init_with_valid_params(self):
        ...
        self.assertEqual(
            self.chatml_tokenizer.response_header_length, 3
        )  # From our mock

    def test_init_with_invalid_params(self):
        """Test initialization with invalid max_tokens_per_sample"""
        with self.assertRaises(ValueError):
            ChatMLTokenizer(tokenizer=self.tokenizer, max_tokens_per_sample=0)
...
class TestSmolTalk(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_chatml_tokenizer = ChatMLTokenizer(
            tokenizer=self.mock_tokenizer, max_tokens_per_sample=100
        )
    ...
    def test_init_with_valid_params(self):
        ...
        dataset = SmolTalk(tokenizer=self.mock_chatml_tokenizer, subset="all")
    ...
    def test_init_with_invalid_subset(self):
        """Test initialization with invalid subset"""
        with self.assertRaises(ValueError):
            SmolTalk(tokenizer=self.mock_chatml_tokenizer, subset="invalid-subset")
...
class TestUltraFeedbackSFT(unittest.TestCase):
    ...
    def test_init_with_valid_params(self):
        ...
        dataset = UltraFeedbackSFT(tokenizer=self.mock_chatml_tokenizer, split="train")
    ...
    def test_init_with_invalid_split(self):
        """Test initialization with invalid split"""
        with self.assertRaises(ValueError):
            UltraFeedbackSFT(tokenizer=self.mock_chatml_tokenizer, split="invalid")
